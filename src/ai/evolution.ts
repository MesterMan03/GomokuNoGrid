...
export async function runEvolution(
    ...
    for (let gen = 0; gen < totalGenerations; gen++) {
        ...
        const fitnesses: number[] = new Array(population.length).fill(0);

        if (matchPool) {
            // ── Parallel: dispatch all matches for this generation ───
            ...
            for (let i = 0; i < population.length; i++) {
                for (let sim = 0; sim < params.simsPerBatch; sim++) {
                    tasks.push({
                        id: nextTaskId++,
                        difficulty,
                        candidateConfig: population[i]!,
                        baselineConfig: definition.defaultConfig,
                        maxMoves: currentMaxMoves,
                        candidateIsPlayer0: sim % 2 === 0,
                        individualIndex: i,
                        gen: gen + 1,
                        sendUpdates: !!onGameUpdate,
                    });
                }
            }

            const results = await matchPool.runMatches(tasks, onGameUpdate);

            // Aggregate fitness per individual using task ID → individual index map
            const taskIdToIndex = new Map<number, number>();
            const fitMap = new Map<number, number[]>();
            for (const task of tasks) {
                taskIdToIndex.set(task.id, task.individualIndex);
                if (!fitMap.has(task.individualIndex)) fitMap.set(task.individualIndex, []);
            }
            for (const result of results) {
                const idx = taskIdToIndex.get(result.id);
                if (idx !== undefined) {
                    fitMap.get(idx)!.push(result.fitness);
                }
            }
            for (const [idx, fits] of fitMap) {
                fitnesses[idx] = fits.reduce((a, b) => a + b, 0) / fits.length;
            }
        } else {
            // ── Sequential: run matches one at a time (fallback) ─────
            ...
            for (let i = 0; i < population.length; i++) {
                ...
                fitnesses[i] = totalFitness / params.simsPerBatch;
            }
        }
